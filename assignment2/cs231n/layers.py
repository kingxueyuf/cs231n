...
def softmax_loss(x, y):
    ...
    - dx: Gradient of the loss with respect to x
    """
#     shifted_logits = x - np.max(x, axis=1, keepdims=True) # [100 , 200, 300] -> [-200, -100, 0]
#     Z = np.sum(np.exp(shifted_logits), axis=1, keepdims=True) # ( -200 + -100 + 0 ) / 3 = -100
#     log_probs = shifted_logits - np.log(Z) #[-200, -100, 0] - (-100) = [-100, 0, 100]
#     probs = np.exp(log_probs) # [e^-100, e^0, e^100]
#     N = x.shape[0]
